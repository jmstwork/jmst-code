 /**
	 ...
	 * author wp
	 */
function myDropDownList(inputId, ajaxUrl, minChars, pageSize, valueId) {
    $("#"+inputId).autocomplete(ajaxUrl, {
        ...
        formatItem: function (row, i, max) {
            ...
            return index + "/" + dataCount + ": \"" + row.sourceTable + "\" [名称:" + row.sourceName + "]";
        },
    ...
    $("#" + inputId).result(function (event, data, formatted) {
        if(data){
        	if(inputId == "sourceTable" || inputId=="sourceName"){
        		$(this).val(data[inputId]);
           	    $("#" + valueId).val(data[valueId]);
           	    if(inputId == "sourceTable"){
           	    	//alert("sourceTableName:"+data[valueId]);
           	    	$("#sourceTableName").val(data[valueId]);
           	    }else{
           	    	//alert("sourceTableName:"+data[inputId]);
           	    	$("#sourceTableName").val(data[inputId]);
           	    }
        	}else if(inputId=="targetTable"){
        		$(this).val(data.sourceTable);
           	    $("#" + valueId).val(data.sourceName);
           	    // alert("targetTableName:"+data.sourceName);
           	    $("#targetTableName").val(data.sourceName);
        	}else if(inputId=="targetName"){
