				...
				$.post("../role/deleteRoleUser.html",{"roleId":$("#roleId").val(),"userIds":userAccounts},function(data){
					if(data.status == '1'){
						asyncbox.success("保存成功！","执行结果",function(){
							var url_ = "../role/roleEdit.html?tabTitle=1&roleId="+$("#roleId").val();
							location.href=url_;
						});
					}else{
...
//	})
	...
	$("#saveBtn").click(function(){
		...
		$.post(url,{"roleId":roleId,"userIds":userIds},function(data){
			if(data.status == '1'){
				asyncbox.success("保存成功！","执行结果",function(){
					var url_ = "111../role/roleEdit?tabTitle=1.html";
					alert(url_);
					location.href=url_;
				})
...
//	});
				...
				if(action == 'save'){
					...
					if(userAccounts==""){
						...
						$.post(url,{"roleId":$("#roleId").val(),"userAccounts":userAccounts},function(data){
							if(data.status == '1'){
								asyncbox.success("保存成功！","执行结果",function(){
									var url_ = "../authority/roleEdit.html?tabTitle=1&roleId="+$("#roleId").val();
									location.href=url_;
								});
							}else{
								asyncbox.error("保存失败！","执行结果",function(){
	...
	function ajaxNext(){
		...
		var jumpToPage=Number(currentPage)+1;
		if((jumpToPage-1)<=totalPage){
			jumpToPage=jumpToPage;
		}else{
			jumpToPage=totalPage;
		}
		var url ="../authority/role/userList.html"
		var param = "roleId="+roleId+"&userAccount="+userAccount
			+"&userName="+userName+"&currentPage="+currentPage+"&totalPage="+totalPage
			+"&jumpToPage="+jumpToPage;
		alert(url+"?"+param);
