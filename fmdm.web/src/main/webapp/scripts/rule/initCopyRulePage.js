...
var specialItem = "";
var content = "";
var updateData="",deleteItem="",deleteData="";
var opt = "";
...
function removeCondition(oId) {
	...
	if(opt == 'edit'){
		if(deleteItem==""){
			deleteItem = jQuery("#column_exe td:nth-child("+(num-3+1)+")")[0].id;
		}else{
			deleteItem += ":"+jQuery("#column_exe td:nth-child("+(num-3+1)+")")[0].id;
		}
	}
...
function editCondition(v) {
	...
	//var url = getRootPath()+"/app/rlmg/initConditionPage.vi?isCon="+jQuery("#"+v).attr("class")+"&modelId="+$("#modelCode").val()+"&desc="+encodeURIComponent(v)+"&exe="+encodeURIComponent(exe+":"+txt);
	var tid = "setCondition";
	if (jQuery("#div_" + v).attr("name") == 'result') {
		url = "../rule/initResultPage.html";
		//url = getRootPath()+"/app/rlmg/initResultPage.vi?modelId="+$("#modelCode").val()+"&desc="+encodeURIComponent(v)+"&exe="+encodeURIComponent(exe+":"+txt);
		tid = "setCondition";
	}
	asyncbox
			.open({
...
function editConditionExe(v) {
	...
	if(opt == 'edit'){
		if(jQuery("#column_exe td:eq("+(n-3)+")")[0].attributes.getNamedItem("class")==null || jQuery("#column_exe td:eq("+(n-3)+")")[0].attributes.getNamedItem("class").value!="add"){
			jQuery("#column_exe td:eq("+(n-3)+")")[0].setAttribute("class","update");
		}
	}
}
...
function deleteRow(obj){
	...
	function saveRule() {
		...
		$.getJSON(check_url, {ruleName:ruleName,'ruleId':$("#ruleId").val()}, function(data) {
			if(data.resultSet != 0){
				asyncbox.error("规则名称重复","执行结果");
				return false;
			}else{
				...
				//alert("updateItems:"+updateItems);
				var modelId= jQuery("#modelCode").val();
				var memo = jQuery("#memo").val();
				var url_ ="../rule/saveCopyRule.html";
				$.getJSON(url_,  {'titles':encodeURI(titles),'exe':encodeURI(exe),
					'data':encodeURI(data),'ruleName':encodeURI(ruleName),'createType':$("#createType").val(),
					'rulegroupId':rulegroupId,'unitId':unitId,'opt':'edit','ruleId':ruleId,
					'modelId':modelId,'memo':encodeURI(memo)}, function(data) {
					if (data.resultSet == 1) {
						//alert(SuccessMsg);
						//returnRule('edit');
						asyncbox.success(SuccessMsg,"执行结果",function(){returnRule('edit');});
					}else{
						//alert(FailMsg);
						asyncbox.error(FailMsg,"执行结果");
	...
	$(function() {
		...
		document.getElementById("memo").value= $("#aaa").val();
