$(function(){
				...
				callback : function(action,opener){
					if(action == 'save'){
						...
						unChecked.each(function(i){
							var id = unChecked[i].parentElement.parentElement.childNodes[5].innerText;
							var name = unChecked[i].parentElement.parentElement.childNodes[7].innerText;
	...
	$("#saveBtn").click(function(){
		...
		}else if($("#opt").val()=="edit"&&$("#userType")=="4"){
			unitId = $("#unitId").val();
		...
		if(isOK){
			$(".panel").slideUp("slow");
			$("#opt").val("editSave");
			$("#editForm").attr("action","../sysmgt/wsOperate.html").submit();
		}
